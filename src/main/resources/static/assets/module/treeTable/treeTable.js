...
layui.define(['layer', 'laytpl', 'form'], function (exports) {
    ...
    TreeTable.prototype.init = function () {
        ...
        viewHtml += '            <table class="layui-table"></table>';
        viewHtml +=this.options.loading? '            <div class="ew-tree-table-loading"><i class="layui-icon layui-anim layui-anim-rotate layui-anim-loop">&#xe63d;</i></div>':'';
        ...
        var $tbLoading =options.loading ?components.$tbLoading:'';  // loading视图
        ...
        if (options.reqData) {  // 异步加载
            ...
            if (options.data && options.data.length > 0) {
                ...
                options.loading&& $tbLoading.hide();
    ...
    TreeTable.prototype.getComponents = function () {
        ...
        var $tbLoading =this.options.loading? $tbBox.children('.ew-tree-table-loading'):"";  // 加载视图
    ...
    TreeTable.prototype.renderBodyAsync = function (d, $tr) {
        ...
        var components = this.getComponents();
        var $tbEmpty = components.$tbEmpty;
        var $tbLoading =options.loading? components.$tbLoading:'';
        // 显示loading
        if ($tr&&options.loading) {
            ...
            if (options.data && options.data.length > 0) {
                options.loading&& $tbLoading.addClass('ew-loading-float');
            }
           options.loading&& $tbLoading.show();
            $tbEmpty.hide();
        }
        // 请求数据
        options.reqData(d, function (res) {
            ...
            if ($tr) {
                ...
                options.loading&& $tbLoading.hide();
                options.loading&& $tbLoading.removeClass('ew-loading-float');
                // 是否为空
                if (!res || res.length == 0) {
                    $tbEmpty.show();
                } else {
    ...
    TreeTable.prototype.refresh = function (id, data) {
        ...
        if (data) {  // 数据模式
            options.loading&& components.$tbLoading.addClass('ew-loading-float');
            options.loading&& components.$tbLoading.show();
            this.renderBodyData(data, d, $tr);
            options.loading&& components.$tbLoading.hide();
            options.loading&&  components.$tbLoading.removeClass('ew-loading-float');
